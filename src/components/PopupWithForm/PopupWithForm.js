    ...
    <div id={props.id} className={props.isOpen ? 'popup popup_opened' : 'popup popup_closed'}>
      <form name={props.id.slice(0, props.id.indexOf('-'))} className="popup__form-container" onSubmit={props.onSubmit}>
        ...
        <fieldset className="popup__fieldset">
          <input id="popup-email-input" name="email" className="popup__text-input" type="email" placeholder="Введите почту" minLength="2" maxLength="40" required />
          <span id="popup-email-input-error" className="popup__input-error popup__input-error_hidden"></span>
          <input id="popup-password-input" name="password" className="popup__text-input" type="password" placeholder="Введите пароль" minLength="8" maxLength="40" required />
