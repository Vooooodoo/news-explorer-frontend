    ...
    <div id={props.id} className={`popup ${props.isOpen ? 'popup_opened' : 'popup_closed'}`}>
      <form name={props.id.slice(0, props.id.indexOf('-'))} className="popup__form-container" onSubmit={props.onSubmit} noValidate>
        ...
        <fieldset className="popup__fieldset">
          <label for={`${props.id}-email-input`} className="popup__input-label">Email</label>
          ...
          <label for={`${props.id}-password-input`} className="popup__input-label">Пароль</label>
          ...
          {props.id === 'registration-popup'
            && (<>
                <label for={`${props.id}-name-input`} className="popup__input-label">Имя</label>
